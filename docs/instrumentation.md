...
For a **reading** Claude stage, the harness creates a detached worktree at the exact reviewed HEAD, outside the implementer's tree. It records the implementer's HEAD and a fingerprint of `git status --porcelain` before dispatch, then checks both again afterwards. A changed HEAD or working-tree fingerprint makes the stage a `contract_violation`. Writes within Claude's disposable review worktree produce a warning and are discarded with that worktree. The stage prompt prohibits file edits, commits, and pushes; `--disallowedTools Edit Write NotebookEdit` adds defence in depth without serving as the guarantee. Read-only Claude dispatches record `read_only_mode = isolated_verified`.

Codex retains `-s read-only` and records `read_only_mode = enforced`, meaning its sandbox prevents workspace writes. Orca still requires its explicit review-workspace contract. Reviews may use any configured eligible executor; a profile may still choose no fallback. Publication access remains behavioural: a read-only stage receives only the tools its role permits, such as `gh` for an allowed comment.
