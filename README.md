...
| `routing.jev.mode`, `routing.jev.model`, `routing.jev.timeout_seconds` | `run_cycle.py` via `jev_shadow.load_jev_config()` | `disabled` (default) or `shadow`. In `shadow`, `implement` and `resolve` stages ask [TypeSafe](https://docs.typesafe.ai/) which of `cheap_coder` or `deep_coder` it would pick and record the answer beside the rules' choice; the suggestion never selects a profile, a target or a dispatch. `model` defaults to `jev-latest` and accepts `jev-latest` or `jev-1.13.0`; `timeout_seconds` defaults to `3` (at most `10`). The key comes from `TYPESAFE_API_KEY`, with `JEV_API_KEY` as a compatibility fallback; without either, the stage records `unavailable`. See [docs/instrumentation.md](docs/instrumentation.md#shadow-suggestions). |
...
`TYPESAFE_API_KEY` (or legacy `JEV_API_KEY`), a timeout or an error only records its category. Any other key
