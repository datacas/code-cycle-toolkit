...
- `cc-implement-issue` diagnoses a work item before editing. It treats the
  ...
  broken invariant, and ends in one decision. A duplicate, or a shared cause
  whose root fix exceeds the item, stops as `BLOCKED` before any branch exists
  and comments on the work item; a root fix within scope lists the other items
  it addresses. The PR gains a *Diagnosis* section and the structured result an
